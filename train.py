...
def load_data(config):
    ...
    train_df = pd.read_csv(
        os.path.join(ABS_PATH, 'data', 'valtrain_data_point_cnn40.csv'), 
        # os.path.join(ABS_PATH, 'data', 'train.csv'), 
        usecols=range(2, 14), #[2, 3, 5],
    )
    test_df = pd.read_csv(
        # os.path.join(ABS_PATH, 'data', 'test_data_point_cnn.csv'), 
        os.path.join(ABS_PATH, 'data', 'test_data_point_cnn40.csv'), 
        usecols=range(2, 14), #[2, 3, 5],
    )
    return (
...
def main():
    ...
    patience, no_improve = 20, 0  # 添加用于早停的patience机制
    no_improve = 0
