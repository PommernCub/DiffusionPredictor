# -*- coding: utf-8 -*-
"""
Created on Fri May 23 13:33:17 2025
...
train_data_outfile = os.path.join(dataset_dir, 
                    "train_data_point_cnn200_smooth.csv")
test_data_outfile = os.path.join(dataset_dir, 
                    "test_data_point_cnn200_smooth.csv")
# 读取文件批次大小
BATCH_SIZE = 200
# 异常检测的关键参数
...
def preprocess_features(stats, file_list):  
    ...
    for file_name in file_list:  # tqdm(file_list, desc="处理数据"):
        try:
            ...
            # # 曲线平滑效果
            # plot_process_data(norm_dist, data["Diffusion Coefficient"])
            # plot_process_data(mean_distance1, data["Diffusion Coefficient"])
            # plot_process_data(auto_corr_coef, data["Diffusion Coefficient"])
            # plot_process_data(abnormal_prob, data["Diffusion Coefficient"])
            # plot_process_data(dist_pow, data["Diffusion Coefficient"])
            # plot_process_data(dist_diff1, data["Diffusion Coefficient"])
            # plot_process_data(fft0, data["Diffusion Coefficient"])
            # plot_process_data(fft1, data["Diffusion Coefficient"])
...
if __name__ == "__main__":
    ...
    # stats = {'total_mean': 0.3599677986334609,
    #  'total_std': 0.1925874877361278,
    #  'total_max': 6.629658651153381,
    #  'total_min': -1.8686554419643533,
    #  'normal_mean': 0.3965649624950076,
    #  'normal_std': 0.2070891237415353,
    #  'abnormal_mean': 0.3233318702458885,
    #  'abnormal_std': 0.1691297553820934}
    # 数据分批量预处理
