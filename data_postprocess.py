# -*- coding: utf-8 -*-
"""
Created on Sun May 25 01:12:32 2025
...
def save_segment_lengths(dir_path, true_dist, predicted_dist, grouped_stats):
    """保存段落长度分布到文件"""
    output_path = os.path.join(dir_path, f"segment_{MODEL}_{SIZE}nm.csv")
    # 合并所有可能的长度值
...
def save_ji_distribution(dir_path, ji_dist, total_n):
    """保存JI指数分布到文件（带频率）"""
    output_path = os.path.join(dir_path, f"ji_{MODEL}_{SIZE}nm.csv")
...
def main(dir_path, input_file):
    # 读取数据
    df = pd.read_csv(os.path.join(dir_path, input_file))
    # 处理两列数据
    # df['true_label_processed'] = process_column(df['true_label'].values)
    # df['predicted_label_processed'] = process_column(df['predicted_label'].values)
    ...
    # 保存结果
    df.to_csv(os.path.join(dir_path, f'processed-{MODEL}-{SIZE}nm.csv'), index=False)
...
if __name__ == "__main__":
    ...
    input_file = f"test-results.csv"
    main(dir_path, input_file)